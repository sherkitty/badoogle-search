...
def auth_required(f):
    ...
    def decorated(*args, **kwargs):
        ...
        badoogle_user = os.getenv('BADOOGLE_USER', '')
        badoogle_pass = os.getenv('BADOOGLE_PASS', '')
        if (not badoogle_user or not badoogle_pass) or (
                auth
                and badoogle_user == auth.username
                and badoogle_pass == auth.password):
            session['auth'] = True
            return f(*args, **kwargs)
        else:
...
def after_request_func(resp):
    ...
    if os.getenv('BADOOGLE_CSP', False):
...
def search():
    ...
    # removing st-card to only use badoogle time selector
...
def run_app() -> None:
    parser = argparse.ArgumentParser(
        description='Badoogle Search console runner')
    parser.add_argument(
        '--port',
        default=5000,
        metavar='<port number>',
    ...
    if args.userpass:
        ...
        os.environ['BADOOGLE_USER'] = user_pass[0]
        os.environ['BADOOGLE_PASS'] = user_pass[1]

    if args.proxytype and args.proxyloc:
        if args.proxyauth:
            proxy_user_pass = args.proxyauth.split(':')
            os.environ['BADOOGLE_PROXY_USER'] = proxy_user_pass[0]
            os.environ['BADOOGLE_PROXY_PASS'] = proxy_user_pass[1]
        os.environ['BADOOGLE_PROXY_TYPE'] = args.proxytype
        os.environ['BADOOGLE_PROXY_LOC'] = args.proxyloc
    ...
    if args.debug:
        ...
        waitress.serve(
            ...
            url_prefix=os.environ.get('BADOOGLE_URL_PREFIX', ''))
