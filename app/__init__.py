...
app.config['DEFAULT_CONFIG'] = os.path.join(
    ...
    'config.json')
app.config['CONFIG_DISABLE'] = read_config_bool('BADOOGLE_CONFIG_DISABLE')
...
# Session values
app_key_path = os.path.join(app.config['CONFIG_PATH'], 'badoogle.key')
...
# link. Setting this value to 'strict' causes Badoogle to revalidate a new
...
app.config['RELEASES_URL'] = 'https://github.com/' \
                             'sherkitty/badoogle-search/releases'
...
# related searches.
translate_url = os.getenv('BADOOGLE_ALT_TL', 'https://farside.link/lingva')
