...
def send_tor_signal(signal: Signal) -> bool:
    use_pass = read_config_bool('BADOOGLE_TOR_USE_PASS')

    confloc = './misc/tor/control.conf'
    # Check that the custom location of conf is real.
    temp = os.getenv('BADOOGLE_TOR_CONF', '')
...
class Request:
    ...
    Attributes:
        ...
        root_path: the root path of the badoogle instance
        config: the user's current badoogle configuration
    """

    def __init__(self, normal_ua, root_path, config: Config):
        self.search_url = 'https://www.google.com/search?gbv=1&num=' + str(
            os.getenv('BADOOGLE_RESULTS_PER_PAGE', 10)) + '&q='
        ...
        proxy_path = os.environ.get('BADOOGLE_PROXY_LOC', '')
        if proxy_path:
            proxy_type = os.environ.get('BADOOGLE_PROXY_TYPE', '')
            proxy_user = os.environ.get('BADOOGLE_PROXY_USER', '')
            proxy_pass = os.environ.get('BADOOGLE_PROXY_PASS', '')
            auth_str = ''
            if proxy_user:
